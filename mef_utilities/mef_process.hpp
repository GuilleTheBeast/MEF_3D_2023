// Función para calcular el volumen en 3D
...
{
    float A = abs(((x2 - x1) * (y3 - y1) * (z4 - z1) + (x3 - x1) * (y4 - y1) * (z2 - z1) + (x4 - x1) * (y2 - y1) * (z3 - z1)) - ((z2 - z1) * (y3 - y1) * (x4 - x1) + (z3 - z1) * (y4 - y1) * (x2 - x1) + (z4 - z1) * (y2 - y1) * (x3 - x1))) / 6;
    return ((A == 0) ? 0.000001 : A);
}
// Función para calcular el jacobiano en 3D
...
{
    float J = ((x2 - x1) * (y3 - y1) * (z4 - z1) + (y2 - y1) * (z3 - z1) * (x4 - x1) + (z2 - z1) * (x3 - x1) * (y4 - y1)) - ((x4 - x1) * (y3 - y1) * (z2 - z1) + (y4 - y1) * (z3 - z1) * (x2 - x1) + (z4 - z1) * (x3 - x1) * (y2 - y1));
...
// Función para crear la K local
void create_local_K(Matrix *K, int element_id, Mesh *M)
{
    // Se establece el tamaño de la matriz
    K->set_size(4, 4);
    // Se establece el problem data
    ...
    // Función para matriz transpuesta
    Matrix Bt(4, 3), At(3, 3);
    transpose(&B, 3, 4, &Bt);
    transpose(&A, 3, 3, &At);
    // Bt.show(); At.show();

    // Multiple multiplicación de escalar por matriz
    ...
    product_scalar_by_matrix(k * volumen / (J * J), &res3, 4, 4, K); // check
...
void create_local_b(Vector *b, int element_id, Mesh *M)
{
    ...
    // Se establece el arreglo de b's
    b->set(Q * J / 24, 0);
    b->set(Q * J / 24, 1);
    b->set(Q * J / 24, 2);
    b->set(Q * J / 24, 3);
...
// Función para crear el sistema local
...
// Función para aplicar Dirichlet
...
// Funcion para resolver el sistema final
...
// Función para unir los resultados con dirichlet
